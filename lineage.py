...
class Event:
    ...
    def canonical_bytes(self) -> bytes:
        ...
        """
        return f"{self.model_id}|{self.index}|{self.prev_hash}|{self.event_hash}|{self.payload_commit}|{self.ts}".encode("utf-8")
...
class LineageStore:
    ...
    def append(self, ev: Event) -> None:
        ...
        if ev.index != expected_idx:
            raise ValueError(f"Index mismatch: ev.index={ev.index}, expected={expected_idx}")

        expected_prev = self.last_hash(ev.model_id)
        if ev.prev_hash != expected_prev:
            raise ValueError(f"prev_hash mismatch for {ev.model_id}: {ev.prev_hash} != {expected_prev}")
...
def rebuild_event(
    ...
    # For now payload_hash == event_hash (unified)
